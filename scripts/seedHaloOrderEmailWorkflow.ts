#!/usr/bin/env bun
/**
 * Seed Halo Order Email Workflow
 *
 * Registers the halo-order-email-send workflow and an event routing rule that
 * triggers it on `halo.order.confirmed` events. The workflow calls halo-api's
 * internal render endpoint and delivers the store-branded buyer receipt and
 * seller notification via Herald.
 *
 * Prereqs (vortex-worker env): HALO_API_URL, HALO_INTERNAL_SERVICE_KEY (must
 * equal halo-api's INTERNAL_SERVICE_KEY), HERALD_API_URL, HERALD_API_KEY.
 *
 * Usage:
 *   VORTEX_API_URL=https://api.vortex.omni.dev \
 *   VORTEX_API_KEY=<key> \
 *   VORTEX_ORGANIZATION_ID=<org-id> \
 *   bun run scripts/seedHaloOrderEmailWorkflow.ts
 */

import template from "../../vortex-worker/src/workflows/templates/halo-order-email-send.json";
...
const WORKFLOW_NAME = "halo-order-email-send";
...
const workflowRes = await fetch(
  `${VORTEX_API_URL}/api/v1/workflows/${WORKFLOW_NAME}`,
  {
    ...
    body: JSON.stringify({
      definition: template,
      description:
        "Send store-branded buyer receipt and seller notification for confirmed Halo storefront orders",
      isActive: true,
    }),
  },
);
...
if (!workflowRes.ok) {
  ...
  console.error(`Failed to upsert workflow: ${workflowRes.status} ${body}`);
  process.exit(1);
}
  ...
  body: JSON.stringify({
    query: `mutation CreateEventRoutingRule($input: CreateEventRoutingRuleInput!) {
      createEventRoutingRule(input: $input) {
        eventRoutingRule { rowId typePattern sourcePattern enabled priority }
      }
    }`,
    variables: {
      input: {
...
const routingResult = (await routingRuleRes.json()) as {
  data?: {
    createEventRoutingRule?: {
      eventRoutingRule?: { rowId: string; typePattern: string };
    };
  };
...
if (routingResult.errors?.length) {
  ...
  if (isDuplicate) {
    console.log("Event routing rule already exists for halo.order.confirmed");
  } else {
    console.error(
      "GraphQL errors:",
  ...
  const rule = routingResult.data?.createEventRoutingRule?.eventRoutingRule;
  console.log(
    `Created event routing rule: ${rule?.typePattern} -> workflow ${workflow.id} (id: ${rule?.rowId})`,
  );
}

console.log("Done seeding halo-order-email-send workflow");
