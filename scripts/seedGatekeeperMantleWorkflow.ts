#!/usr/bin/env bun
/**
 * Seed Gatekeeper-to-Mantle User Linking Workflow
 *
 * Registers the gatekeeper-user-to-mantle workflow and an event routing rule
 * that triggers it on `gatekeeper.user.created` events.
 *
 * Usage:
 *   VORTEX_API_URL=https://api.vortex.omni.dev \
 *   VORTEX_API_KEY=<key> \
 *   VORTEX_ORGANIZATION_ID=<org-id> \
 *   bun run scripts/seedGatekeeperMantleWorkflow.ts
 */

import template from "../../vortex-worker/src/workflows/templates/gatekeeper-user-to-mantle.json";
...
const WORKFLOW_NAME = "gatekeeper-user-to-mantle";
...
const workflowRes = await fetch(
	`${VORTEX_API_URL}/api/v1/workflows/${WORKFLOW_NAME}`,
	{
		...
		body: JSON.stringify({
			definition: template,
			description:
				"Auto-link Gatekeeper users to Mantle contacts on signup",
			isActive: true,
		}),
	},
);
...
if (routingResult.errors?.length) {
	...
	if (isDuplicate) {
		console.log(
			"Event routing rule already exists for gatekeeper.user.created",
		);
	} else {
		console.error(
			"GraphQL errors:",
...
console.log("Done seeding gatekeeper-user-to-mantle workflow");
