...
const internalRoutes = new Elysia({ prefix: "/internal" })
  ...
  .post("/cleanup-stale-runs", async ({ headers, status }) => {
    ...
    // Runs older than 1 hour in non-terminal status are considered stale
    // (aligned with the background reaper in lib/triggers/reaper.ts)
    ...
    const staleRuns = await db
      ...
      .where(
        and(
          ...
          lte(workflowRunTable.startedAt, cutoff),
        ),
      )
