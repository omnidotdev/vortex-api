    ...
    routes: [
      {
        ...
        priority: 10,
        // Route only owner-carrying fractal events that have an email template:
        // build.failed, service.crashed, service.resource_warning. Owner-less
        // events (service.reconciled, rebuild_triggered, backup.*) can never
        // resolve a recipient, and deploy.succeeded is routine success with no
        // template (it 400d Herald on every deploy), so both are filtered out
        // here to avoid spawning a workflow run per event (a firehose that
        // crashed the worker)
        celCondition:
          'has(event.data.owner) && event.data.owner != "" && event.type != "fractal.deploy.succeeded"',
      },
    ],
  },
  {
