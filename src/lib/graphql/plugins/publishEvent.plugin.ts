...
export const executePublishEvent = async (
  ...
  // Batch-fetch all workflows referenced by matching rules
  const workflowIds = [...new Set(matchingRules.map((r) => r.workflowId))];
  let workflowsById: Map<
    string,
    NonNullable<Awaited<ReturnType<typeof db.query.workflowTable.findFirst>>>
  >;
  ...
  if (workflowIds.length === 0) {
    ...
    try {
      const workflows = await db.query.workflowTable.findMany({
        where: drizzleAnd(
          inArray(wfTable.id, workflowIds),
          drizzleEq(wfTable.isActive, true),
        ),
      });
      workflowsById = new Map(workflows.map((w) => [w.id, w]));
    } catch (err) {
      logger.error("Failed to batch-fetch workflows for routing rules", {
        eventId,
        workflowIds,
        error: err instanceof Error ? err.message : String(err),
      });
      ...
      throw new GraphQLError(
        "Failed to fetch workflows for event routing. Please try again later.",
        { extensions: { code: "EVENT_ROUTING_ERROR" } },
      );
    }
  ...
  for (const rule of matchingRules) {
    const workflow = workflowsById.get(rule.workflowId);
