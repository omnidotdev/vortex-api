...
async function pollWorkflow(workflow: {
  ...
  try {
    ...
    if (!(await isExecutionAllowed(workflow.organizationId))) {
      ...
      logger.warn("Polling trigger skipped: monthly execution limit reached", {
        workflowId: workflow.id,
    ...
    const [run] = await db
      ...
      .values({
        workflowId: workflow.id,
        engineWorkflowId,
        engineRunId: `run-${crypto.randomUUID()}`,
        status: "pending",
...
const LOCK_KEY = "vortex:polling-scheduler:lock";
const LOCK_TTL_S = 90; // slightly longer than SCAN_INTERVAL_MS

/**
 * Try to acquire a distributed lock. Returns true if acquired.
 */
async function acquireLock(): Promise<boolean> {
  if (!cacheClient) return true; // single instance, no lock needed
  const result = await cacheClient.set(LOCK_KEY, "1", "EX", LOCK_TTL_S, "NX");
  return result === "OK";
}
...
async function scanPollingWorkflows(): Promise<void> {
  if (!(await acquireLock())) return; // another instance holds the lock
