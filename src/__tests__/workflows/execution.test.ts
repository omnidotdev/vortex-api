...
const mockReturning = mock(() => {
  const pending = (mockReturning as any).__pendingRow;
...
const mockInsertValues = mock((values: RunRow) => {
  ...
  (mockReturning as any).__pendingRow = row;
  return { returning: mockReturning };
});

const mockInsert = mock(() => ({ values: mockInsertValues }));

const mockUpdateWhere = mock(async (condition: unknown) => {
  // Apply pending set to matching rows (simplified: update first match)
  const pending = (mockUpdateWhere as any).__pendingSet as RunRow;
...
const mockUpdateSet = mock((values: RunRow) => {
  (mockUpdateWhere as any).__pendingSet = values;
